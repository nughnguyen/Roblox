...
local NughFolder = "Nugh"
local ConfigurationFolder = NughFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		nughOpen = {Type = 'bind', Value = 'K', Name = 'GumballZ Keybind'},
		-- buildwarnings
		-- nughprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(NughFolder) then
				if isfile and isfile(NughFolder..'/settings'..ConfigurationExtension) then
					file = readfile(NughFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"nughOpen":{"Value":"K","Type":"bind","Name":"GumballZ Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"GumballZ Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('Nugh had an issue accessing configuration saving capability.')
		end
	end
end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("Nugh", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify Nugh developers at sirius.menu/discord.")
				end
			end)
...
local Nugh = useStudio and script.Parent:FindFirstChild('Nugh') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if Nugh:FindFirstChild('Build') and Nugh.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Nugh | Build Mismatch')
		print('Nugh may encounter issues as you are running an incompatible interface version ('.. ((Nugh:FindFirstChild('Build') and Nugh.Build.Value) or 'No Build') ..').\n\nThis version of Nugh is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Nugh = Nugh, useStudio and script.Parent:FindFirstChild('Nugh') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(Nugh)
	Nugh.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Nugh.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Nugh.Name and Interface ~= Nugh then
			Interface.Enabled = false
			Interface.Name = "Nugh-Old"
		end
	end
elseif not useStudio then
...
if Nugh.AbsoluteSize.X < minSize.X and Nugh.AbsoluteSize.Y < minSize.Y then
...
local Main = Nugh.Main
local MPrompt = Nugh:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Nugh:FindFirstChild('Drag')
...
local Notifications = Nugh.Notifications

local SelectedTheme = NughLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = NughLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Nugh.Main.BackgroundColor3 = SelectedTheme.Background
	Nugh.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Nugh.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Nugh.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Nugh.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Nugh.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Nugh.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Nugh.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Nugh.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Nugh:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Nugh | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Nugh | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Nugh had an issue decoding the configuration file, please try delete the file and reopen Nugh.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(NughLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Nugh | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--NughLibrary:Notify({Title = "Nugh Flags", Content = "Nugh was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(NughLibrary.Flags) do
...
function NughLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Nugh | Not seeing your actions in notifications?')
			print("Notification Actions are being sunset for now, keep up to date on when they're back in the discord. (dsc.gg/thenoicez)")
		end
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			NughLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show Nugh'.", Duration = 7, Image = 4400697855})
		else
			NughLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.nughOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(NughFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Nugh Settings', 0, true)

	if TabList['Nugh Settings'] then
		TabList['Nugh Settings'].LayoutOrder = 1000
	end

	if Elements['Nugh Settings'] then
		Elements['Nugh Settings'].LayoutOrder = 1000
	end
...
function NughLibrary:CreateWindow(Settings)
	if Nugh:FindFirstChild('Loading') then
		if getgenv and not getgenv().nughCached then
			Nugh.Enabled = true
			Nugh.Loading.Visible = true

			task.wait(1.4)
			Nugh.Loading.Visible = false
		end
	end

	if getgenv then getgenv().nughCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				NughLibrary:Notify({Title = 'Build Mismatch', Content = 'Nugh may encounter issues as you are running an incompatible interface version ('.. ((Nugh:FindFirstChild('Build') and Nugh.Build.Value) or 'No Build') ..').\n\nThis version of Nugh is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(NughFolder) then
		makefolder(NughFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Nugh"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Nugh Interface Suite" then
		LoadingFrame.Version.Text = "GumballZ UI"
	end
	...
	if not Settings.DisableNughPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				NughLibrary:Notify({
					Title = "Nugh Interface",
					Content = "Enjoying this UI library? Find it at dsc.gg/thenoicez",
					Duration = 7,
					Image = 4370033185,
				})
			end
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(NughFolder.."/Discord Invites") then
			makefolder(NughFolder.."/Discord Invites")
		end

		if isfile and not isfile(NughFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(NughFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Nugh RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(NughFolder.."/Key System") then
			makefolder(NughFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Nugh | "..Key.." Error " ..tostring(Response))
					warn('Check dsc.gg/thenoicez for help with QuocHung specific development.')
				end
			end
		end
		...
		if isfile and isfile(NughFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(NughFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(NughFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						NughLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Nugh | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check dsc.gg/thenoicez for help with QuocHung specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= NughLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					NughLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Nugh.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Nugh | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check dsc.gg/thenoicez for help with QuocHung specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					NughLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Nugh | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check dsc.gg/thenoicez for help with QuocHung specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					NughLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Nugh | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check dsc.gg/thenoicez for help with QuocHung specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					NughLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= NughLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Nugh | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check dsc.gg/thenoicez for help with QuocHung specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						NughLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Nugh | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check dsc.gg/thenoicez for help with QuocHung specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					NughLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			NughLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			NughLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Nugh had an issue creating settings.') end
...
function NughLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function NughLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function NughLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Nugh Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.nughOpen.Value or 'K'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function NughLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				NughLibrary:Notify({Title = "Nugh Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			NughLibrary:Notify({Title = "Nugh Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Nugh Configurations Error | '..tostring(result))
			NughLibrary:Notify({Title = "Nugh Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = NughLibrary:CreateWindow({
		Name = "Nugh Example Window",
		LoadingTitle = "Nugh Interface Suite",
		Theme = 'Default',
		Icon = 0,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using Nugh may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Nugh to get the key from
	...
	--NughLibrary:Notify({Title = "Nugh Interface", Content = "Welcome to Nugh. These - are the brand new notification design for Nugh, with custom sizing and Nugh calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(NughLibrary.Theme) do
